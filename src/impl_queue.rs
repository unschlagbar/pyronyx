...
impl Queue {
    ...
    #[inline]
    pub fn submit(&self, submits: &[SubmitInfo], fence: Fence) -> Result<(), vkResult> {
        unsafe {
            (self.fns().v1_0.queue_submit.unwrap())(
                self.handle,
                submits.len() as u32,
                submits.as_ptr(),
                fence,
    ...
    pub fn wait_idle(&self) -> Result<(), vkResult> {
        unsafe { (self.fns().v1_0.queue_wait_idle.unwrap())(self.handle) }.result()
    }

    /// <https://docs.vulkan.org/refpages/latest/refpages/source/vkQueueBindSparse.html>
    #[inline]
    pub fn bind_sparse(&self, bind_info: &[BindSparseInfo], fence: Fence) -> Result<(), vkResult> {
        unsafe {
            (self.fns().v1_0.queue_bind_sparse.unwrap())(
                self.handle,
    ...
    #[inline]
    pub fn submit2(&self, submits: &[SubmitInfo2], fence: Fence) -> Result<(), vkResult> {
        unsafe {
            (self.fns().v1_3.queue_submit2.unwrap())(
                self.handle,
                submits.len() as u32,
                submits.as_ptr(),
                fence,
